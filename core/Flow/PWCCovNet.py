...
class CovFeature(nn.Module):

    def __init__(self, md=4, flow_norm=20.0):
        super(CovFeature, self).__init__()
        # cnet
        self.pwc = PWCNet(md=md, flow_norm=flow_norm)
    ...
    def load_tartanvo_weight(self, path):
        ...
        for key in checkpoint:
            if key.startswith("module.flowNet."):
                converted_checkpoint[key[15:]] = checkpoint[key]
            elif key.startswith("module.flowPoseNet"):
                continue
            elif key.startswith("module."):
                converted_checkpoint[key[7:]] = checkpoint[key]
            else:
