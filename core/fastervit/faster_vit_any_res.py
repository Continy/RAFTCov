...
default_cfgs = {
    ...
    _cfg(
        url=
        'https://huggingface.co/ahatamiz/FasterViT/resolve/main/fastervit_0_224_1k.pth.tar',
        crop_pct=0.875,
        input_size=(3, 224, 224),
        crop_mode='center'),
}
...
def window_partition(x, window_size):
    B, C, H, W = x.shape
    x = x.view(B, C, H // window_size, window_size, W // window_size,
               window_size)
...
def ct_window(ct, W, H, window_size):
    bs = ct.shape[0]
    N = ct.shape[2]
    ct = ct.view(bs, H // window_size, window_size, W // window_size,
                 window_size, N)
    ...
    def forward(self, input_tensor, local_window_size):
        ...
        if not self.grid_exists:
            ...
            relative_position_bias = torch.nn.functional.pad(
                ...
                (n_global_feature, 0, n_global_feature, 0)).contiguous()
            if n_global_feature > 0 and self.ct_correct:
...
class PosEmbMLPSwinv1D(nn.Module):
    ...
    def __init__(self, dim, rank=2, seq_length=4, conv=False):
        ...
        if not conv:
            ...
            self.cpb_mlp = nn.Sequential(
                nn.Conv1d(self.rank, 512, 1, bias=True), nn.ReLU(),
    ...
    def forward(self, input_tensor):
        seq_length = input_tensor.shape[
            1] if not self.conv else input_tensor.shape[2]
        if self.deploy:
        ...
        if not self.grid_exists:
            self.grid_exists = True
            if self.rank == 1:
                relative_coords_h = torch.arange(0,
                                                 seq_length,
                ...
                relative_coords_h = torch.arange(0,
                                                 ...
                                                 device=input_tensor.device,
                                                 dtype=input_tensor.dtype)
                relative_coords_w = torch.arange(0,
                                                 seq_length,
                                                 device=input_tensor.device,
                                                 dtype=input_tensor.dtype)
                relative_coords_table = torch.stack(
...
class PatchEmbed(nn.Module):
    ...
    def __init__(self, in_chans=3, in_dim=64, dim=96):
        ...
        self.conv_down = nn.Sequential(
            ...
            nn.BatchNorm2d(in_dim, eps=1e-4), nn.ReLU(),
            nn.Conv2d(in_dim, dim, 3, 2, 1, bias=False),
            nn.BatchNorm2d(dim, eps=1e-4), nn.ReLU())
...
class HAT(nn.Module):
    ...
    def __init__(self,
        ...
        self.do_sr_hat = True if ((sr_ratio[0] > 1) or
                                  (sr_ratio[1] > 1)) else False
        ...
        self.gamma3 = nn.Parameter(layer_scale *
                                   torch.ones(dim)) if use_layer_scale else 1
        self.gamma4 = nn.Parameter(layer_scale *
                                   torch.ones(dim)) if use_layer_scale else 1
        ...
        if self.do_sr_hat:
            ...
            self.hat_attn = WindowAttention(dim,
                                            num_heads=num_heads,
                                            qkv_bias=qkv_bias,
                                            qk_scale=qk_scale,
                                            attn_drop=attn_drop,
            ...
            self.hat_mlp = Mlp(in_features=dim,
                               hidden_features=mlp_hidden_dim,
                               act_layer=act_layer,
                               drop=drop)
    ...
    def forward(self, x, carrier_tokens):
        ...
        if self.do_sr_hat:
            ...
            ct = ct_dewindow(ct, self.cr_window * self.sr_ratio[0],
                             self.cr_window * self.sr_ratio[1], self.cr_window)
            ...
            ct = ct_window(ct, self.cr_window * self.sr_ratio[0],
                           self.cr_window * self.sr_ratio[1], self.cr_window)
        ...
        if self.do_sr_hat:
            ...
            if self.last and self.do_propagation:
                ...
                x = x + self.gamma1 * self.upsampler(
                    ctr_image_space.to(dtype=torch.float32)).flatten(
                        2).transpose(1, 2).to(dtype=x.dtype)
        return x, ct
...
class TokenInitializer(nn.Module):
    ...
    def __init__(self, dim, input_resolution, window_size, ct_size=1):
        ...
        to_global_feature.add_module(
            "pool",
            nn.AvgPool2d(kernel_size=(kernel_size1, kernel_size2),
...
class FasterViTLayer(nn.Module):
    ...
    def __init__(self,
        ...
        if len(
                self.blocks
        ) and not only_local and sr_ratio and hierarchy and not self.conv:
