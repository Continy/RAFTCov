...
i = list(range(5001, 60002, 5000))
aepe_mse = np.load('results/5001/aepe_mse.npy')
aepe_mse = np.array(aepe_mse)
aepe_mse = np.mean(aepe_mse, axis=0)
aepe_var_all = []
for j in i:
    aepe_var = np.load('results/' + str(j) + '/aepe_var.npy')
    aepe_var = np.array(aepe_var)
...
plt.legend(loc='best')
plt.savefig('results/common_sparsification_plot.png')
plt.show()
