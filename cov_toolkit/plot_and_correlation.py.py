...
aepe_mse = np.load('results/new/5001/aepe_mse.npy')
aepe_mse = np.array(aepe_mse)
aepe_mse = np.mean(aepe_mse, axis=0)
aepe_var_all = []
for j in i:
    aepe_var = np.load('results/new/' + str(j) + '/aepe_var.npy')
    aepe_var = np.array(aepe_var)
...
mid_var = aepe_var[(aepe_var.shape[0] - 1) // 2]
mid_mse = aepe_mse[(aepe_mse.shape[0] - 1) // 2]
...
fig.tight_layout()
plt.savefig('results/new/curves.png')
plt.show()
plt.figure()
plt.plot(x, mid_var, label='Estimated Variance', color='blue', linewidth=2)
plt.plot(x, mid_mse, label='Oracle', color='red', linewidth=2, linestyle='--')
plt.legend(loc='best')
plt.savefig('results/new/common_sparsification_plot.png')
plt.show()
