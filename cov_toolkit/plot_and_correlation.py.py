...
aepe_mse = np.load(datapath + '5001/aepe_mse.npy')
aepe_mse = np.array(aepe_mse)
aepe_mse = np.mean(aepe_mse, axis=0)
aepe_var_all = []
for j in i:
    aepe_var = np.load(datapath + str(j) + '/aepe_var.npy')
    aepe_var = np.array(aepe_var)
...
fig.tight_layout()
plt.savefig(datapath + 'curves.png')
plt.show()
plt.figure()
...
plt.legend(loc='best')
plt.savefig(datapath + 'common_sparsification_plot.png')
plt.show()
